...
#  if defined (HPRESMEL)
      presmel(zh) = rhomp*grav*(HPRESMEL**2)   ! steep rise for zh 
     *          * (max(zh-HPRESMEL,0.)/10.)    ! > HPRESMEL (by ~10 m's)
c b  *          * (max(zh-HPRESMEL,0.)/30.)    ! > HPRESMEL (by ~10 m's)
c a  *          * (max(zh-HPRESMEL,0.)/10.)**3 ! > HPRESMEL (by ~10 m's)
#  else
      presmel(zh) = 0.
#  endif
