...
c     air-temp cutoff for rainfall vs. snowfall (in iceclimall.F):
      ...
      parameter (rhoman = 3370., rhobed = 2550., rhosed = 2050.) ! kg/m3
#else
      parameter (rhoman = 3370., rhobed = 3370., rhosed = 2390)  ! kg/m3
#endif
...
c        For sediment domain-wide diagnostic budget
      ...
      common /sedbud/ 
     *  totquar, totpelag, totzerosed, totdump, totslump
...
#if defined (CRHHARD)
      parameter (crhhard= CRHHARD)                ! hard bedrock
#elif defined (RESOL80) || defined (RESOL160)
      parameter (crhhard= 1.e-11)                 ! hard bedrock 
#else
      parameter (crhhard= 1.e-10)                 ! hard bedrock 
#endif

#if defined (CRHSED)
      parameter (crhsed = CRHSED)                 ! (new) sediment 
#else
      parameter (crhsed = 1.e-5)                  ! (new) sediment 
#endif

#if defined (CRHREGO)
      parameter (crhrego = CRHREGO)            ! pre 40 Ma land regolith
#else
      parameter (crhrego = 1.e-9)              ! pre 40 Ma land regolith
#endif

#if defined (REGOLITHINIT)
      parameter (regolithinit = REGOLITHINIT)  ! pre 40 Ma land regolith
#else
      parameter (regolithinit = 20.)           ! pre 40 Ma land regolith
#endif

#if defined (CRHSEDO)
      parameter (crhsedo = CRHSEDO)            ! pre 40 Ma ocean sed
#else
      parameter (crhsedo = 1.e-6)              ! pre 40 Ma ocean sed
#endif

#if defined (SEDOCEANINIT)                  
      parameter (sedoceaninit = SEDOCEANINIT)  ! pre 40 Ma ocean sed
#else
      parameter (sedoceaninit = 100.)          ! pre 40 Ma ocean sed
#endif
...
c        For Last Interglacial atmos and ocean perturb forcing (as DP16)
      ...
      data timlig /-130000., -125000., -120000., -115000. /
