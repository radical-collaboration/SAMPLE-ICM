...
c+++++++++++++++++++++++++++
      ...
      parameter (dlatd =   0.5,       dlond =  1.0)       ! nh
      parameter (alon1 = -180.,       alon2 = 180.)       ! nh
      parameter (alat1 =   20.,       alat2 =  85.)       ! nh
...
#      else
      parameter (dx0=40.e3, dy0=dx0,  dd0=dx0)
#      endif
      parameter (nx =2500.e3/dx0 + .001, ny=1)
#    elif defined (LINEE)
...
c     parameter (nx=40,     ny=35)
      parameter (nx=45,     ny=45)
#        else
...
#      elif defined (RESOL80)
      parameter (dx0=80.e3, dy0=dx0,  dd0=dx0)
#        if defined (FORCE34MYR)
c     parameter (nx=70,     ny=70)  ! runabce
c     parameter (nx=90,     ny=70)  ! rund
      parameter (nx=90,     ny=90)
#        else
...
#  elif defined (LOVECLIP)
      parameter (nlong=64,  nlatg=32)                      ! atmos (T21)
      parameter (nlongo=180, nlatgo=90)                    ! ocean
...
#  if defined (LOVECLIP)
     *                ,darlc(nlong,nlatg), 
     *                 alongo(nlongo), alatgo(nlatgo), 
     *                 darlco(nlongo,nlatgo)
#  endif
...
c        For 1-D flowline strip output in iceshow1d and iceplot1d
      ...
      character*80 c1s
      common /c1ddisp/ i1s(nx+ny), j1s(nx+ny), d1s(nx+ny), n1s, c1s 

c>>>>>>>>>>>>>>>>>>>>>
c        For sediment tracking in sedtracking_eul (and on restart file).
c        ntrack*dtrack must be >= the thickest sediment encountered in 
c        the run.
c>>>>>>>>>>>>>>>>>>>>>
#if defined (SEDTRACK)
      parameter (ntrack = 1000, dtrack = 10.)
#else
      parameter (ntrack =    1, dtrack = 1.)
#endif
      parameter (vnulsedtrack = -9999.)
      common /csedtrack/ 
     *  sedtrack(nx,ny,0:ntrack), itrtop(nx,ny)
c>>>>>>>>>>>>>>>>>>>>>
...
c       Number of ocean sectors for ocean melt (OCMARTIN)
      ...
      parameter (nsec=4)
      character*16 csector(nsec)
      common /comsec/ csector
...
c     for sediment sector volumes (Wilson et al., P3, 2012, Table 3).
c     Values for totsedsecobs,alonsedsec,csedsec in data stmts,
c     in calcwilsonbasins (icesed.F):
      parameter (nsedsec=8)
      character*12 csedsec(nsedsec)
      common /csedsec/ totsedsec(nsedsec), totsedsecobs(nsedsec), 
     *                 totqacsec(nsedsec), 
     *                 alonsedsec(0:nsedsec), msedsec(nx,ny), csedsec
